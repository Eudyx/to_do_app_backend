...
const createNewBoard = async (req = request, res) => {
  ...
  if (!boardName || !boardName?.length)
    return res.status(400).json({ message: "The board name is required." });
...
try {
    ...
    if (!foundBoard)
      return res.status(404).json({ message: `ID ${id} is Not match.` });
...
const addingNewSection = async (req, res) => {
  if (!req?.body?.id || !req.body.sectionName)
    return res
      .status(400)
      .json({ message: "ID and section name are required" });
...
const deleteSection = async (req, res) => {
  ...
  if (!id || !bID)
    return res.status(400).json({ message: "Id and board id are required." });

  const foundBoard = await Board.findOne({ _id: bID });

  const foundSection = foundBoard.sections.filter((section) => {
    if (section._id.toString() === id) return section;
  });

  if (!foundSection?.length) return res.sendStatus(204);

  const deletedSection = foundBoard.sections.filter(
    (section) => section._id !== foundSection[0]._id
  );
...
const addingNewTask = async (req, res) => {
  if (
    ...
    return res.status(400).json({ message: "It is missing a field" });
  ...
  if (!foundSection)
    return res
      .status(204)
      .json({ message: `The ID ${req.body.sectionID} was not found.` });
...
const deleteTask = async (req, res) => {
  ...
  const foundBoard = await Board.findOne({ _id: bID });
  const foundTask = foundBoard.tasks.filter((task) => {
    if (task._id.toString() === id) return task;
  });

  if (!foundTask?.length) return res.sendStatus(204);

  const deletedTask = foundBoard.tasks.filter(
    (task) => task._id !== foundTask[0]._id
  );
